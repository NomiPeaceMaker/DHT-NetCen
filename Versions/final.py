...
sockforfilesdownload = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
...
def sendMessage (sendMsg, friend1): # Saara masla yahan hai!
    try:
...
def faizan(message, sockold):
    ...
    if message["purpose"] == "sendhere":
        if message["download"] == "yes":
            sendFilefordownload(message["filename"],message["message"], message["from"], message["download"])
        elif message["download"] != "yes":
            sendFile(message["filename"],message["message"], message["from"], message["download"])
    if message["purpose"] == "downloadrequest":
        filehere = amisucc(message["message"],pred,port)
        if filehere == True:
            print("I have the file and gt ready to recieve!")
            sockforfiles = sockold
            sendMsg = {"purpose": "recievefiledownload", "message": message["message"], "from": port, "filename": message["filename"], "download": "yes"}
            sendMessage(sendMsg, message["from"])
        elif(filehere == False):
            sendMessage(message, succ)
    if message["purpose"] == "recievefiledownload":
        print("downloading the file I asked for")
        sockforfiles = sockold
        recieveFilefordownload(message["from"], message, sockforfiles)
...
def sendFile(filename, hashValue, port2send2, download): 
    global sockforfiles
    try:
...
def sendFilefordownload(filename, hashValue, port2send2, download):
    global sockforfilesdownload
    try:
        sockforfilesdownload.connect(('localhost', port2send2))
        print("sneding")
        with open(filename, 'rb') as f:
            sent = sockforfilesdownload.sendfile(f,0)
            print("bytes sent: ", sent)
        if(exit1==True):
            exit()
    except ConnectionRefusedError:
        ...
        print("closing file transfer socket")
        # time.sleep(3)
        sockforfilesdownload.close()

#######################################################################################################################


def recieveFilefordownload(port1, message, sock):
    sockforfilesdownload = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sockforfilesdownload.bind(('localhost', 8194))
    sendMsg = {"purpose": "sendhere", "message": message["message"], "from": 8194, "filename": message["filename"], "download": message["download"]}
    ...
    try:
        ...
        sockforfilesdownload.listen(1)
        print("listening!")
        sendMessage(sendMsg, port1)
        inSock = sockforfilesdownload.accept()[0]
        f= open(filename,'wb')
        i=0
        while exit1 == False:
            l= inSock.recv(1024)
...
def recieveFile(port1, message, sock):
    ...
    sendMsg = {"purpose": "sendhere", "message": message["message"], "from": 8193, "filename": message["filename"], "download": message["download"]}
    ...
    while exit1==False:
        ...
        if choice == 1:
            ...
            if filehere == True:
                ...
                sendMsg = {"purpose": "recievefile", "message": hashValue, "from": port, "filename": filename, "download": "no"}
                sendMessage(sendMsg, succ)
        if choice == 2:
            ...
            filehere = amisucc(hashValue,pred,port)
            if filehere == True:
                print("file hashes to your port and either you have it or it hasn't been uploaded")
            elif filehere == False:
